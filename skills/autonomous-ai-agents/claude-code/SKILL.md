...
- **Kimi Code wrapper:** `npm install -g kimi-code` — proxies Kimi/Moonshot models through Claude Code's interface (starts anthropic-proxy with Kimi model, then runs claude-code)
- **Auth:** run `claude` once to log in (browser OAuth for Pro/Max, or set `ANTHROPIC_API_KEY`)
- **Console auth:** `claude auth login --console` for API key billing
- **SSO auth:** `claude auth login --sso` for Enterprise
- **OpenRouter proxy:** use `--bare` + `ANTHROPIC_API_KEY` from OpenRouter. See `references/openrouter-proxy.md`.
